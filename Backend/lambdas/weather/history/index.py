...
def lambda_handler(event, context):
    ...
    try:
        ...
        if 'lat' not in params or 'lng' not in params or 'dt' not in params:
            return {
                'statusCode': 400,
                'headers': CORS_HEADERS,
                'body': json.dumps(ErrorResponse(
                    error='BadRequest',
                    message='Missing required parameters: lat, lng, dt',
                    timestamp=get_iso_timestamp()
                ).dict())
            }
        ...
        with httpx.Client(timeout=WEATHER_API_TIMEOUT) as client:
            response = client.get(
                ...
                params={"key": WEATHER_API_KEY, "q": f"{lat},{lng}", "dt": dt}
            )
